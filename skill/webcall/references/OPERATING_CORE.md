...
4. An archive sent into a **fresh** conversation with no message gets a model
   ...
   naming the archive and `000_READ_ME_FIRST.md`; the extension holds it on
   the handoff and types it into the composer only **after** the operator's
   attachment has landed, so the archive and the instruction travel together.
   Not in conductor mode — a thread already in use has the context, and it is
   the operator's conversation. If typing fails the panel offers the text to
   copy. The operator still clicks Send.
   `[companion/core.py launch_prompt; extension/service_worker.js
   typeLaunchPrompt, completeAttachment]`
